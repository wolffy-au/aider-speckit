...
class TestSpeckitCommands(TestCase):
    ...
    def test_sanitize_constitution_keeps_latest_h1_section(self):
        raw = "# Old\nold body\n\n# New\nnew body"
        result = SpeckitCommandsMixin._sanitize_constitution_text(raw)
    ...
    def test_cmd_speckit_specify_rejects_malformed_response(self):
        with GitTemporaryDirectory() as repo_dir:
            ...
            checklist_placeholder = spec_dir / "checklists" / "requirements.md"
            self.assertTrue(checklist_placeholder.exists())
            self.assertTrue(any(os.path.samefile(spec_file, fname) for fname in coder.abs_fnames))
            self.assertTrue(
                any(os.path.samefile(checklist_placeholder, fname) for fname in coder.abs_fnames)
            )

    def test_cmd_speckit_specify_retries_when_assistant_requests_files(self):
        with GitTemporaryDirectory() as repo_dir:
            os.chdir(repo_dir)
            template_path = Path(repo_dir) / ".aider" / "commands" / "speckit.specify.md"
            template_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            placeholder_response = (
                "I need to update the generated feature spec and quality checklist. "
                "Please add specs/001-photo-organizer/spec.md and "
                "checklists/requirements.md so I can continue."
            )
            final_response = "# Feature Specification: Photo Organizer\n\n## Content\n"

            initial_coder = mock.Mock()
            initial_coder.run.return_value = placeholder_response
            fallback_coder = mock.Mock()
            fallback_coder.run.return_value = final_response

            with mock.patch(
                "aider.coders.base_coder.Coder.create",
                side_effect=[initial_coder, fallback_coder],
            ):
                with mock.patch.object(
                    SpeckitCommandsMixin, "_generate_short_name", return_value="photo-organizer"
                ):
                    with mock.patch.object(
                        ...
                        with mock.patch.object(
                            ...
                            with mock.patch.object(
                                SpeckitCommandsMixin,
                                "_build_spec_generation_prompt",
                                return_value="fallback prompt",
                            ) as mock_build_prompt:
                                commands.cmd_speckit_specify("Create organizational spec")

            fallback_coder.run.assert_called_once_with("fallback prompt")
            mock_build_prompt.assert_called_once()

            self.assertTrue(spec_file.exists())
            self.assertEqual(final_response, spec_file.read_text())
